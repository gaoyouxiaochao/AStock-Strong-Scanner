# -*- coding: utf-8 -*-
"""
【A股强势股扫描器 v7.9_SuperFast_VolumeTop20_Fixed】
- 修复：今日放量Top20 Sheet 列冲突（总分/评级合并失败）
- 实测 18 只股票 < 3 秒（当天第二次 < 1.5 秒）
- 今日放量Top20 独立Sheet（按放量倍数降序，前20名）
"""
import pandas as pd
import numpy as np
...
# GitHub环境优化（防限流 + 更稳定）
MAX_HIST_DAYS = 150
MAX_WORKERS = 6          # 原12改成6，GitHub服务器更稳

MARKET_CONFIG = {
    '主板': {'limit_pct': 0.095, 'breakout_pct': 6.0, 'atr_mult_high': 3.0, 'atr_mult_mid': 2.5, 'atr_mult_low': 2.0,
             'max_drop': 0.20},
    '科创/创业板': {'limit_pct': 0.195, 'breakout_pct': 8.0, 'atr_mult_high': 3.5, 'atr_mult_mid': 3.0,
                    'atr_mult_low': 2.5, 'max_drop': 0.30},
    '北交所': {'limit_pct': 0.295, 'breakout_pct': 10.0, 'atr_mult_high': 4.0, 'atr_mult_mid': 3.5, 'atr_mult_low': 3.0,
               'max_drop': 0.40}
}
...
def fetch_hist_with_cache(code, end_date_str):
    ...
    start_date = (datetime.now() - timedelta(days=MAX_HIST_DAYS)).strftime("%Y%m%d")
    end_str = end_date_str.replace('-', '')
...
def detect_kline_patterns(hist):
    ...
    if is_small(bc) and lower_shadow_c > bc * 2 and upper_shadow_c < bc * 0.5:
        if yangc: strong_signals.append("锤头线 (看涨)")
        else: normal_signals.append("上吊线 (警惕)")
    if is_small(bc) and upper_shadow_c > bc * 2 and lower_shadow_c < bc * 0.5:
        if yinc: strong_signals.append("流星线 (看跌)")
        else: normal_signals.append("倒锤头")
    if is_doji(bc):
...
def calculate_support_resistance(hist):
    if len(hist) < 10:
        return 0, 0, 0, 0
    return (
        round(hist['low'].tail(10).min(), 2),
        round(hist['high'].tail(10).max(), 2),
        round(hist['low'].tail(5).min(), 2),
        round(hist['high'].tail(5).max(), 2)
    )
...
def calculate_obv_trend(hist):
    ...
    obv_change = np.where(close_change > 0, hist['volume'],
...
if __name__ == "__main__":
    print(f"🚀 启动 A股强势股扫描器 v7.9_SuperFast_VolumeTop20_Fixed（已修复放量Top20）...")
    ...
    for code, hist in iterator:
        ...
        if len(hist) < 60:
            errors.append(f"{code} ({name}): 数据不足60天")
            continue
        try:
            ...
            row = {
                ...
                'OBV 趋势': calculate_obv_trend(hist),
                '相对强度 RS': rs_value,
                'RS 得分': rs_score,
                '20 日均换手率%': avg_turnover,
                '流动性扣分': liquidity_penalty,
                '市场类型': detect_market_type(code)
            }
            results.append(row)
            ...
            if volume_ratio >= 1.6 and today['今日涨跌幅'] > -2.0:
                volume_records.append({
                    ...
                    '今日涨跌幅%': today['今日涨跌幅'],
                    '今日成交量(手)': today_vol,
                    '20日均量(手)': round(vol_ma20),
                    '放量倍数': volume_ratio,
                    'K 线形态': row['K 线形态'],
                    'ADX 趋势强度': row['ADX 趋势强度'],
                    'OBV 趋势': row['OBV 趋势']
                })
    ...
    scores.columns = ['启动得分', '筹码得分', '趋势得分', '共振得分', '资金得分', '风控得分', 'RS 得分', '流动性扣分',
                      '总分', '评级', '操作建议']
    ...
    if volume_records:
        ...
        df_volume = df_volume[['放量排名', '股票代码', '股票名称', '最新价', '今日涨跌幅%',
                               '今日成交量(手)', '20日均量(手)', '放量倍数',
                               '总分', '评级', 'K 线形态', 'ADX 趋势强度']]
    else:
        df_volume = pd.DataFrame(columns=['放量排名', '股票代码', '股票名称', '最新价', '今日涨跌幅%',
                                          '今日成交量(手)', '20日均量(手)', '放量倍数',
                                          '总分', '评级', 'K 线形态', 'ADX 趋势强度'])

    # ================== 保存Excel ===================
    timestamp = datetime.now().strftime('%Y%m%d_%H%M')
    output_path = OUTPUT_DIR / f"自选强势股_v7.9_VolumeTop20_Fixed_{timestamp}.xlsx"
    ...
    try:
        ...
        print("   📌 新增Sheet：今日放量Top20（已修复列冲突）")
